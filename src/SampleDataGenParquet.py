...
4. Parquet output for seamless Spark/Databricks ingestion
"""
...
# CONFIGURATION
# ============================================================
OUTPUT_DIR = "/Volumes/databricks-poc-updated/nike_poc/data_volume/nike_sample_data_parquet"
...
def generate_dim_promotion(dim_date: pd.DataFrame) -> pd.DataFrame:
    ...
    for promo_sk in range(1, NUM_PROMOTIONS + 1):
        ...
        end_date_sk = start_date_sk + duration_days
...
def generate_fact_sales_transactions(
    ...
    # Get valid keys
    date_sks = dim_date['date_sk'].tolist()
    customer_sks = [None] + dim_customer['customer_sk'].tolist()  # None for guest checkout
    product_data = dim_product[['product_sk', 'standard_retail_price', 'standard_cost', 'division_code', 'category_name']].to_dict('records')
    store_data = dim_store[['store_sk', 'region_code', 'is_digital']].to_dict('records')
    channel_sks = dim_channel['channel_sk'].tolist()
    promotion_sks = [None] + dim_promotion['promotion_sk'].tolist()  # None for no promotion
    employee_sks = [None] + dim_employee[dim_employee['is_active']]['employee_sk'].tolist()
    ...
    for i in range(num_unique_transactions):
        ...
        # Select appropriate channel based on store type
        if store['is_digital']:
            channel_sk = np.random.choice([2, 3, 4])  # Digital channels
        else:
            channel_sk = np.random.choice([1, 5, 6, 7, 8])  # Physical channels
        
        # Customer selection (20% guest checkout)
        customer_sk = np.random.choice(customer_sks) if np.random.random() > 0.2 else None
        
        # Employee (only for physical stores)
        employee_sk = np.random.choice(employee_sks) if not store['is_digital'] else None
...
def generate_fact_inventory_snapshot(
    ...
    sample_stores = dim_store[dim_store['is_digital'] == False].sample(min(100, len(dim_store)))['store_sk'].tolist()
...
def generate_fact_web_sessions(
    ...
    date_sks = dim_date['date_sk'].tolist()
    customer_sks = [None] + dim_customer['customer_sk'].tolist()
    ...
    for date_sk in date_sks:
        ...
        for _ in range(daily_sessions):
            ...
            # 40% of sessions are from known customers
            customer_sk = np.random.choice(customer_sks) if np.random.random() < 0.4 else None
...
def save_to_parquet(dataframes: Dict[str, pd.DataFrame], output_dir: str):
    """Save all dataframes to Parquet files using Spark."""
    print(f"\nSaving Parquet files to {output_dir}/")
    for table_name, df in dataframes.items():
        filepath = f"{output_dir}/{table_name}"
        sdf = spark.createDataFrame(df)
        sdf.write.mode("overwrite").parquet(filepath)
        print(f"  Saved {table_name} as parquet ({len(df):,} rows)")
...
def main():
    ...
    save_to_parquet(dataframes, OUTPUT_DIR)

    print("\n" + "="*60)
    print("✅ DATA GENERATION COMPLETE!")
    print("="*60)
    print(f"\nAll Parquet files saved to: {OUTPUT_DIR}/")
    print("\nTable Summary:")
    print("-"*40)
    for table_name, df in dataframes.items():
